...
const generateSampleRows = (toDateStr: string, count = 31): DailyRow[] => {
  const toDate = new Date(toDateStr)
  return Array.from({ length: count }).map((_, idx) => {
    const d = new Date(toDate.getTime() - idx * 24 * 60 * 60 * 1000)
    return {
      date: d.toISOString().split('T')[0],
      chargeEnergy: Math.round(100 + idx * 3.5),
      dischargeEnergy: Math.round(80 + idx * 2.7),
      roundTripEfficiency: Math.round(80 + (idx % 10)),
      energyLoss: Math.round((idx % 5) * 2),
      chargingCost: Math.round((idx + 1) * 2.5),
      dischargeRevenue: Math.round((idx + 1) * 3.1),
    }
  })
}
...
export default function Page() {
  ...
  const allRows = useMemo(() => generateSampleRows(to, 31), [to])
  const totalItems = allRows.length
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize))

  const [page, setPage] = useState(totalPages)
  const [gotoValue, setGotoValue] = useState(String(totalPages))
        ...
        <div className="p-6">
          <div className="rounded-lg border border-[#e6edf5] bg-white p-4 mb-6">
            <div className="flex items-center gap-4">
              ...
              <button className="inline-flex h-9 items-center gap-2 rounded bg-[#0f6fff] px-4 text-[13px] font-medium text-white"><Search className="w-4 h-4"/>Search</button>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-[#e6edf5] bg-white p-4">
            <div className="flex justify-end items-center gap-3 mb-2">
              ...
              <button className="h-9 rounded bg-[#0f6fff] px-3 text-sm text-white inline-flex items-center gap-2"><Download className="w-4 h-4"/>Export O&M Daily Report</button>
            </div>
