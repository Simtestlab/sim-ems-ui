...
const generateYearsBetween = (fromY: string, toY: string): YearlyRow[] => {
  let start = Number(fromY)
  let end = Number(toY)
  ...
  for (let y = end; y >= start; y--) {
    ...
    rows.push({
      year: String(y),
      chargeEnergy: Math.round(100 + idx * 12),
      dischargeEnergy: Math.round(80 + idx * 9),
      roundTripEfficiency: Math.round(80 + (idx % 10)),
      energyLoss: Math.round((idx % 5) * 4),
      chargingCost: Math.round((idx + 1) * 25),
      dischargeRevenue: Math.round((idx + 1) * 34),
    })
  }
  return rows
}
...
export default function Page() {
  ...
  const allRows = useMemo(() => generateYearsBetween(from, to), [from, to])
          ...
          <div className="rounded-lg border border-[#e6edf5] bg-white p-4 mb-6">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <input type="number" value={from} onChange={(e)=>setFrom(e.target.value)} className="h-9 w-28 rounded border border-[#dce4ee] px-3 text-[13px]" />
                <span className="text-sm text-[#6b7280]">To</span>
                <input type="number" value={to} onChange={(e)=>setTo(e.target.value)} className="h-9 w-28 rounded border border-[#dce4ee] px-3 text-[13px]" />
              </div>
                ...
                <tbody>
                  {pagedRows.map((r, i) => (
                    <tr key={(page - 1) * pageSize + i} className="border-t border-[#f1f5f9] hover:bg-[#f8fafc]">
                      <td className="py-4 px-4">{r.year}</td>
