...
export function AlertsColumn({
  ...
  return (
    <div 
      className="bg-white border border-gray-200 rounded-2xl p-4 flex-none flex flex-col" 
      style={minHeightPx ? { height: `${minHeightPx}px` } : undefined}
    >
      ...
      <div className="flex-1 mt-1 overflow-y-auto pr-2 min-h-0">
        {items.map((it) => (
          <AlertItem 
            key={it.id} 
            item={it} 
