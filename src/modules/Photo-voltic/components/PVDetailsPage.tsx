...
function closestByHour<T extends { hour: number }>(samples: T[], targetHour: number | null) {
  ...
  // If pointer is before the first sample, snap to first. If after last, snap to last.
...
function buildTelemetrySnapshot(deviceId: string, selectedDate: string, now: Date): TelemetrySnapshot {
  ...
  const powerSeries: LineSeries[] = [
    ...
    },
    {
      ...
      fillId: 'power-intensity-fill',
    },
  ]

  // Build 10 branch series and paginate them 4 per page (4/4/2)
  ...
  const branchSamples = Array.from({ length: totalBranches }, (_, i) => {
    ...
    return {
      ...
      color: BRANCH_COLORS[i % BRANCH_COLORS.length] ?? '#cfd3d9',
  ...
  for (let p = 0; p < Math.ceil(totalBranches / pageSize); p += 1) {
    ...
    branchPages.push({
      plottedSeries: pageSlice.slice(0, 3).map((series, index) => ({
        label: series.label,
        color: series.color,
        points: series.points,
        fillId: index === 0 ? 'current-branch-fill' : undefined,
      })),
      legendSeries: pageSlice.map((series) => ({
        label: series.label,
        color: series.color,
        points: series.points,
        muted: series.muted,
...
function ChartPlot({
  ...
  function handlePointerMove(event: React.MouseEvent<SVGSVGElement>) {
    ...
    // Build union of available sample hours across visible series
    ...
    // Snap to nearest 0.25 hour increment across full day (0-23)
    // This allows tooltip to show even in unplotted areas
  ...
  // Keep tooltip positioned when hover values change programmatically
  useEffect(() => {
  ...
  return (
    <div className="relative h-full">
      ...
      >
        <defs>
          <linearGradient id="power-intensity-fill" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor="#f4c08f" stopOpacity="0.45" />
            <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
          </linearGradient>
          <linearGradient id="current-branch-fill" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor="#7cd4b0" stopOpacity="0.5" />
            <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
          </linearGradient>
        </defs>

        {/* Unit labels positioned in SVG coordinate space to avoid overlap */}
        ...
        {series.map((item) => {
          ...
          return (
            <g key={item.label} transform={`translate(${leftPadding} ${topPadding})`}>
              {areaPath ? <path d={areaPath} fill={`url(#${item.fillId})`} /> : null}
              <polyline
                fill="none"
      ...
      {tooltipTitle && tooltipItems && tooltipPos ? (
        <div
          ...
          style={{ left: tooltipPos.left, top: tooltipPos.top, transform: 'translate(-50%, -120%)' }}
        >
...
export default function PVDetailsPage({ params }: { params?: { id?: string } }) {
  ...
  const id = decodeURIComponent(params?.id ?? '1#PV')
  ...
  // Add PV Details to navigation history (remember last visited inverter id)
  useEffect(() => {
    const route = `/pv/pvdetails?id=${encodeURIComponent(id)}`
    addVisitedTab(route)
  }, [addVisitedTab, id])
  ...
  useEffect(() => {
    // Initialize visibility map when device changes or telemetry become available
    ...
    setVisibleSeries((prev) => {
      // preserve existing toggles when possible, otherwise default: power series visible,
      // branches 01-04 enabled by default and others disabled
  ...
  // breadcrumb is route-driven; inverter name should not be a breadcrumb tag
              ...
              <div className="col-span-12 xl:col-span-6">
                <div className="rounded-[20px] border border-[#e6edf5] bg-white px-5 py-5 shadow-[0_6px_24px_rgba(15,23,42,0.04)] flex flex-col min-h-[460px] h-[460px]">
                  ...
                  <div className="mb-3 flex items-center">
                    <ChartLegend series={telemetry.powerSeries} visibleMap={visibleSeries} onToggle={toggleSeries} />
                  </div>
              ...
              <div className="col-span-12 xl:col-span-6">
                <div className="rounded-[20px] border border-[#e6edf5] bg-white px-5 py-5 shadow-[0_6px_24px_rgba(15,23,42,0.04)] flex flex-col min-h-[460px] h-[460px]">
                  ...
                  <div className="mb-3 flex items-center justify-between gap-4">
                    <ChartLegend series={currentLegendSeries} visibleMap={visibleSeries} onToggle={toggleSeries} />
