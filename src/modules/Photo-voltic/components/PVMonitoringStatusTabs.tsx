...
type PVMonitoringStatusTabsProps = {
...
export default function PVMonitoringStatusTabs({ statusTabs, selectedStatus, counts, onSelectStatus }: PVMonitoringStatusTabsProps) {
  return (
    <div className="px-4 border-b border-[#e9eef5] bg-white sticky top-0 z-10 overflow-visible shadow-sm">
      <div className="flex items-center gap-5 text-[14px] whitespace-nowrap min-w-max w-full">
        {statusTabs.map(tab => (
          <button key={tab.key} onClick={() => onSelectStatus(tab.key)} className={`py-3 px-0 relative transition-all group whitespace-nowrap ${selectedStatus === tab.key ? 'text-[#2f8cf0] font-semibold' : 'text-gray-500 hover:text-gray-800'}`}>
            <span className="flex items-center gap-1.5 whitespace-nowrap">
              {tab.label}
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${selectedStatus === tab.key ? 'bg-[#eef5ff] text-[#7aa6e8]' : 'bg-[#f2f4f7] text-[#9aa6b5]'}`}>{counts[tab.key] ?? 0}</span>
            </span>
